app.controller("mvcCRUDCtrl", function ($scope, crudWorkLogsService) {
    $scope.divBook = false;
    GetAllWorkLogs();
    //To Get all book records  
    function GetAllWorkLogs() {
        var getWorkData = crudWorkLogsService.getWorkLogs();
        getWorkData.then(function (logs) {
            $scope.worklog = logs.data;
        }, function () {
            alert('Error in getting work logs records');
        });
    }
